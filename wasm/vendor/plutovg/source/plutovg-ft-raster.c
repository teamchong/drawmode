...
#define SCALED( x )  (x)
  ...
  gray_convert_glyph_inner( RAS_ARG )
  {
    ...
    /* If cell buffer overflowed (ras.invalid set by gray_record_cell),
       report memory overflow so PVG_FT_Raster_Render retries with a
       larger buffer. This replaces the setjmp/longjmp mechanism. */
    if ( ras.invalid && ras.num_cells >= ras.max_cells )
