...
static int attributeid(const char* data, size_t length)
{
    static const name_entry_t table[] = {
        ...
        {"dominant-baseline", ATTR_DOMINANT_BASELINE},
        {"fill", ATTR_FILL},
...
typedef struct element {
    ...
    const char* text_content;  // For <text>/<tspan>: pointer into SVG data
...
plutosvg_document_t* plutosvg_document_load_from_data(const char* data, int length, float width, float height, plutovg_destroy_func_t destroy_func, void* closure)
{
    ...
    while(it < end) {
        if(current == NULL) {
            ...
            const char* text_start = it;
            while(it < end && *it != '<') {
                ++it;
            }
            // Capture text content for <text>/<tspan> elements
            if(current && (current->id == TAG_TEXT || current->id == TAG_TSPAN)
               && current->text_content == NULL && it > text_start)
            {
                current->text_content = text_start;
                current->text_length = (int)(it - text_start);
            }
        }
...
static void render_text(const element_t* element, const render_context_t* context, render_state_t* state)
{
    if(is_display_none(element) || is_visibility_hidden(element))
        return;
    ...
    parse_length(element, ATTR_X, &x, true, false);
    parse_length(element, ATTR_Y, &y, true, false);
    float _x = resolve_length(&new_state, &x, 'x');
    float _y = resolve_length(&new_state, &y, 'y');

    // Parse font-size (default 16px)
    float font_size = 16.f;
    const string_t* fs_attr = find_attribute(element, ATTR_FONT_SIZE, true);
    if(fs_attr && fs_attr->length > 0) {
        float parsed = 0.f;
        const char* p = fs_attr->data;
        while(p < fs_attr->data + fs_attr->length && (*p >= '0' && *p <= '9' || *p == '.')) {
            if(*p == '.') {
                float frac = 0.f, div = 10.f;
                ++p;
                while(p < fs_attr->data + fs_attr->length && *p >= '0' && *p <= '9') {
                    frac += (*p - '0') / div;
                    div *= 10.f;
                    ++p;
                }
            ...
            parsed = parsed * 10.f + (*p - '0');
            ++p;
        }
        if(parsed > 0.f) font_size = parsed;
    }

    // Parse text-anchor (start, middle, end)
    int anchor = 0; // 0=start, 1=middle, 2=end
    const string_t* ta_attr = find_attribute(element, ATTR_TEXT_ANCHOR, true);
    if(ta_attr) {
        if(ta_attr->length == 6 && memcmp(ta_attr->data, "middle", 6) == 0) anchor = 1;
        else if(ta_attr->length == 3 && memcmp(ta_attr->data, "end", 3) == 0) anchor = 2;
    }

    // Parse dominant-baseline
    int baseline = 0; // 0=default (alphabetic), 1=text-before-edge (top)
    const string_t* db_attr = find_attribute(element, ATTR_DOMINANT_BASELINE, false);
    if(db_attr && db_attr->length >= 16 && memcmp(db_attr->data, "text-before-edge", 16) == 0)
        baseline = 1;

    // Parse fill color
    color_t fill_color = {0xFF000000}; // default black
    parse_color(element, ATTR_FILL, &fill_color, true);
    float fill_opacity = 1.f;
    parse_number(element, ATTR_FILL_OPACITY, &fill_opacity, true, true);
    float opacity = 1.f;
    parse_number(element, ATTR_OPACITY, &opacity, true, true);

    // Select font — try font-family, fall back to first available
    const string_t* ff_attr = find_attribute(element, ATTR_FONT_FAMILY, true);
    if(ff_attr && ff_attr->length > 0) {
        // Extract first font name (before comma)
        char family[64];
        int flen = 0;
        const char* p = ff_attr->data;
        // Skip leading quote if present
        if(*p == '"' || *p == '\'') ++p;
        while(p < ff_attr->data + ff_attr->length && flen < 63 && *p != ',' && *p != '"' && *p != '\'') {
            family[flen++] = *p++;
        }
        family[flen] = '\0';
        plutovg_canvas_select_font_face(context->canvas, family, false, false);
    }
    plutovg_canvas_set_font_size(context->canvas, font_size);
    ...
    // Set fill color
    plutovg_color_t color = convert_color(&fill_color);
    color.a *= fill_opacity * opacity * new_state.opacity;
    plutovg_canvas_set_color(context->canvas, &color);

    // Render this element's direct text content
    if(element->text_content && element->text_length > 0) {
        float draw_x = _x;
        float draw_y = _y;
        ...
        if(anchor != 0) {
            ...
            plutovg_font_face_t* face = plutovg_canvas_get_font_face(context->canvas);
            if(face) {
                plutovg_font_face_text_extents(face, font_size, element->text_content, element->text_length, PLUTOVG_TEXT_ENCODING_UTF8, &text_extents);
                if(anchor == 1) draw_x -= text_extents.w / 2.f;
                else if(anchor == 2) draw_x -= text_extents.w;
            }
        }

        // Adjust for dominant-baseline: text-before-edge means y is the top of text
        if(baseline == 1) {
            plutovg_font_face_t* face = plutovg_canvas_get_font_face(context->canvas);
            if(face) {
                float ascent = 0, descent = 0, line_gap = 0;
                plutovg_font_face_get_metrics(face, font_size, &ascent, &descent, &line_gap, NULL);
                draw_y += ascent;
            }
        }

        plutovg_canvas_fill_text(context->canvas, element->text_content, element->text_length, PLUTOVG_TEXT_ENCODING_UTF8, draw_x, draw_y);
    }

    // Render child <tspan> elements
    const element_t* child = element->first_child;
    while(child) {
        if(child->id == TAG_TSPAN && child->text_content && child->text_length > 0) {
            // tspan can override x, y
            length_t tx = x, ty = y;
            parse_length(child, ATTR_X, &tx, true, false);
            parse_length(child, ATTR_Y, &ty, true, false);
            float tsx = resolve_length(&new_state, &tx, 'x');
            float tsy = resolve_length(&new_state, &ty, 'y');
            ...
            // Parse child fill color (may override parent)
            color_t child_fill = fill_color;
            parse_color(child, ATTR_FILL, &child_fill, false);
            plutovg_color_t child_color = convert_color(&child_fill);
            child_color.a *= fill_opacity * opacity * new_state.opacity;
            plutovg_canvas_set_color(context->canvas, &child_color);
            ...
            if(anchor != 0) {
                ...
                if(face) {
                    plutovg_rect_t text_extents;
                    plutovg_font_face_text_extents(face, font_size, child->text_content, child->text_length, PLUTOVG_TEXT_ENCODING_UTF8, &text_extents);
                    if(anchor == 1) draw_x -= text_extents.w / 2.f;
                    else if(anchor == 2) draw_x -= text_extents.w;
                }
            }

            plutovg_canvas_fill_text(context->canvas, child->text_content, child->text_length, PLUTOVG_TEXT_ENCODING_UTF8, draw_x, tsy);
        }
        child = child->next_sibling;
    }
...
static void render_element(const element_t* element, const render_context_t* context, render_state_t* state)
{
    ...
    case TAG_TEXT:
        ...
        break;
    case TAG_TSPAN:
        break; // handled inside render_text
    }
}
