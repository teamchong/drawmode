...
| `png` | PNG image at 2x resolution | All (built-in via WASM) |
| `svg` | SVG markup | All (built-in via linkedom) |
...
**PNG/SVG export fails?** Ensure the WASM module is built: `pnpm build`. PNG/SVG export uses linkedom + PlutoSVG WASM — no browser or external dependencies needed.
